...
class QMediaPlayer(MediaPlayer):
    ...
    def generate(self, monitors=[]):
        (init, indexes) = super().generate(monitors)
        if fnc.Intervalinmin and indexes:
            self.switch(indexes)
            if init:
                return
        ...
        if source == "cacher":
            ...
            if result == "_submit":
                ...
                if result.startswith("file"):
                    self.tray.showmessage("Downloaded", "Click to view it.", result)
                elif result.startswith("https"):
                    self.tray.showmessage(
                        "Oversized", "Click to download it manually.", result
                    )
        elif source == "checker":
...
class Tray:
    ...
    def answerclick(self, reason):
        # refreshes the stats in the manual sub-menu if right-clicked
...
def configuredialog(player):
    ...
    def saveandexit():
        ...
        with open(fnc.CONFIGURATION, "w", encoding="utf-8") as file:
